...
internal sealed class BrokerDispatcher(ITransportBus bus, IBrokerContext context) : BaseDispatcher(context), IBrokerCommandSender, IBrokerRequestSender, IBrokerPublisher
{
    ...
    public override async ValueTask<TResponse> SendCommandWait<T, TResponse>(T message)
    {
        try
        {
            return await bus.SendAsync<T, TResponse>(message, Context.CancellationToken);
        }
        catch (Exception e)
        {
            throw new DispatchException("Send failed", e)
    ...
    public override async ValueTask<TResponse> SendQuery<T, TResponse>(T message)
    {
        try
        {
            return await bus.QueryAsync<T, TResponse>(message, Context.CancellationToken);
        }
        catch (Exception e)
        {
